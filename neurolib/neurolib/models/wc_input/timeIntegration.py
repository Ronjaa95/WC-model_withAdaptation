...
def timeIntegration(params):
    ...
    step_current = np.zeros((N, startind + len(t))) #external step-current input
    
    #the last but one second is set to maximum for one second 
    if params["step_current"]:
        sec1_neg = 10000 * params['neg_from_second']
        sec2_neg = 10000 * params['neg_to_second']
        sec1 = 10000 * params['from_second']
        sec2 = 10000 * params['to_second']
        step_current[:,-sec1_neg:-sec2_neg] = params['neg_ext_val']
        step_current[:,-sec1:-sec2] = params['ext_val']
...
def timeIntegration_njit_elementwise(
    ...
    return t, excs, inhs, adaps, step_current, exc_ou, inh_ou